...
// NewGenDriftIgnoreCmd creates a new command instance.
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			if opts.OutputPath != "-" {
				...
				defer func() { _ = ignoreFile.Close() }()
				fmt.Fprintf(os.Stderr, "Appending ignore rules to %s\n", opts.OutputPath)
			}
			_, _ = fmt.Fprintf(ignoreFile, "# Generated by gen-driftignore cmd @ %s\n%s\n", time.Now().Format(time.RFC1123), list)
